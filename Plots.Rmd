...
width_data =  read.table("clipboard",header=T,sep="\t") #upload data
...
sum_ben_data = ben_data %>%
  group_by(site) %>%
  summarise(temp_mean = mean(log(benthic_chl),na.rm=T),
...
width = width_data[width_data$site == ind_site, 2]

sim_exp = sum_exp_data %>%
  filter(site==ind_site) 

sim_exp_data = rlnorm(1000,
...
sim_ben_data = rlnorm(1000,
                  as.numeric(sim_ben[1,2]),
                  as.numeric(sim_ben[1,3]))
 
temp = as.data.frame(replicate(1000, (sample(sim_exp_data,1,replace=FALSE))/(width*(sample(sim_ben_data,1,replace=FALSE)))))
      #since I use grams exported then I don't have to transform into km, the result is already in km

temp$site = print(ind_site)
colnames(temp) = c("estimated_length","site")
...
results_matrix$estimated_length = as.numeric(results_matrix$estimated_length)
...
results_matrix_subset =  results_matrix %>%
  filter(site == "WLOU") 
ggplot(results_matrix_subset, aes(x=log10(estimated_length))) + 
  geom_histogram(color="black", fill="white") +
  geom_vline(aes(xintercept=log10(4.5)),
