...
alg_carbon = read.csv(paste("R:/EcosystemEcologyLab/MSAPLANKTONdataDirectory/DataInDevelopment/NEONData/",site,"/04_product/",initial_parameter,".csv",sep=""))
...
Csummary = alg_carbon %>%
  group_by(algalSampleType,collectDate) %>%
  summarise(parameter = mean(algalConc, na.rm=T)) %>%
  mutate(site = print(site)) #in mgC per m2 OR L
