...
3) calculate the rate of decay in [chl] after the peak
...
#calculate the rate of decay in Chl concentration per storm event 
decayChl = AllCQData %>%
  group_by(storm_id) %>%
  filter(row_number() >= (which.max(conc))) %>%
  mutate(time = as.numeric(seq(0,(n()-1)*30,by=30))) %>%
  do(decay = as.numeric(coef(lm(log(conc) ~ time,data=.))[2]))

decayChl = as.data.frame(decayChl)
AllCQData_v4 = left_join(AllCQData_v3,decayChl,by="storm_id")
...
subset_CQData = AllCQData_v4 %>%
  ...
  dplyr::select(start,totalChlexport,deltaChl,decay) 
