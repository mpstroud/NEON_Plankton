...
write.csv(geo_gaugeWaterColumnRegression, 
          "geo_gaugeWaterColumnRegression.csv", 
          row.names=F)
...
############################################################################################################################
#"csd_continuousDischarge" file contains the actual flow data and can be extremely large for some sites, isolate parameters before downloading
write.csv(csd_continuousDischarge, #this is a long file, it will crash most of the times
          "csd_continuousDischarge.csv", 
          row.names=F)
...
for (i in 1:6){
     ...
     temp_d = subset(wquant_data, year==years[i])
      write.csv(temp_d,paste(years[i],"_wquant_data_QApass.csv",sep=""))
...
summary(as.factor(temp_data$regressionID)) #to assess what anual rating curve is being used
